...
public class Runnable implements java.lang.Runnable {
    ...
    public void refresh(String filePath, Long beforeTime) throws Exception {
        ...
        for (int i = 0; i < refreshs.size(); i++) {
            System.out.println("Refresh file: "
                    + mappingPath
                    + StringUtils
                            .substringAfterLast(refreshs.get(i).getAbsolutePath(), mappingPath));
            log.debug("refresh file:" + refreshs.get(i).getAbsolutePath());
            log.debug("refresh filename:" + refreshs.get(i).getName());
            SqlSessionFactoryBean.refresh(new FileInputStream(refreshs.get(i)), refreshs.get(i)
    ...
    @Override
    public void run() {
        location = location.substring("file [".length(), location.lastIndexOf(mappingPath)
                + mappingPath.length());
