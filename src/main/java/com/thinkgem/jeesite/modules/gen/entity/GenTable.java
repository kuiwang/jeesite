...
public class GenTable extends DataEntity<GenTable> {
        ...
        for (GenTableColumn column : getColumnList()) {
            if (column.getIsNotBaseField()
                    || ("1".equals(column.getIsQuery()) && "between".equals(column.getQueryType()) && ("createDate"
                            .equals(column.getSimpleJavaField()) || "updateDate".equals(column
                            .getSimpleJavaField())))) {
                // 导入类型依赖包， 如果类型中包含“.”，则需要导入引用。
                if ((org.apache.commons.lang3.StringUtils.indexOf(column.getJavaType(), ".") != -1)
            ...
            if (column.getIsNotBaseField()) {
                ...
                for (String ann : column.getAnnotationList()) {
                    if (!importList.contains(org.apache.commons.lang3.StringUtils
                            .substringBeforeLast(ann, "("))) {
                        importList.add(org.apache.commons.lang3.StringUtils.substringBeforeLast(
                                ann, "("));
                    }
                }
            }
    ...
    public Boolean getParentExists() {
        return (parent != null) && org.apache.commons.lang3.StringUtils.isNotBlank(parentTable)
                && org.apache.commons.lang3.StringUtils.isNotBlank(parentTableFk);
    }
