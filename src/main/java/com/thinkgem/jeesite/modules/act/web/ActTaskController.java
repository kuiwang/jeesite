...
public class ActTaskController extends BaseController {
     ...
     * @param vars 任务流程变量，如下 vars.keys=flag,pass vars.values=1,true
     *        vars.types=S,B @see
    ...
    public String complete(Act act) {
        actTaskService.complete(act.getTaskId(), act.getProcInsId(), act.getComment(), act
                .getVars().getVariableMap());
        return "true";//adminPath + "/act/task";
    }
    ...
    public String histoicFlow(Act act, String startAct, String endAct, Model model) {
        if (StringUtils.isNotBlank(act.getProcInsId())) {
            List<Act> histoicFlowList = actTaskService.histoicFlowList(act.getProcInsId(),
                    startAct, endAct);
    ...
    public String historicList(Act act, HttpServletRequest request, HttpServletResponse response,
     ...
     *                       procDefId, HttpServletResponse response)
     *                       throws Exception { ProcessDefinition procDef =
     *                       repositoryService
     *                       .createProcessDefinitionQuery(
     *                       ).processDefinitionId
     *                       (procDefId).singleResult(); String
     *                       diagramResourceName =
     *                       procDef.getDiagramResourceName(); InputStream
     *                       imageStream =
     *                       repositoryService.getResourceAsStream
     *                       (procDef.getDeploymentId(),
     *                       diagramResourceName); byte[] b = new
     *                       byte[1024]; int len = -1; while ((len =
     ...
     * 获取跟踪信息
     * 
     * @RequestMapping(value = "processMap") public String
     *                       processMap(String procDefId, String proInstId,
     *                       Model model) throws Exception {
     *                       List<ActivityImpl> actImpls = new
     ...
     *                       流程标识 ProcessDefinitionEntity def =
     *                       (ProcessDefinitionEntity)
     *                       ((RepositoryServiceImpl) repositoryService)
     *                       .getDeployedProcessDefinition
     *                       (processDefinitionId); List<ActivityImpl>
     *                       activitiList = def.getActivities();//
     *                       获得当前任务的所有节点 List<String> activeActivityIds =
     *                       runtimeService
     *                       .getActiveActivityIds(proInstId); for (String
     *                       activeId : activeActivityIds) { for
     *                       (ActivityImpl activityImpl : activitiList) {
     *                       String id = activityImpl.getId(); if
